...
class ShareRepository:
    ...
    def upser_note_share(self, note_id: int, user_id: int, role: str) -> NoteShare:
    ...
    def remove_note_share(self, note_id: int, user_id: int) -> None:
        self.db.exec(delete(NoteShare).where(
            NoteShare.note_id==note_id, NoteShare.user_id==user_id))
        self.db.commit()

    def upser_label_share(self, label_id: int, user_id: int, role: str) -> LabelShare:
        share = self.db.exec(select(LabelShare).where(
            LabelShare.label_id==label_id, LabelShare.user_id==user_id)).first()
    ...
    def list_note_ids_share_directly(self, user_id: int) -> list[int]:
        return self.db.exec(
            select(NoteShare.note_id).where(NoteShare.user_id==user_id)
        ).all()
        #return [ row[0] if isinstance(row, tuple) else row for row in rows]

    def list_label_ids_share_directly(self, user_id: int) -> list[int]:
        return self.db.exec(
